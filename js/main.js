...
let newStr = str.replace(reg, '').toLowerCase().split(' '). filter((item) => item.indexOf(subStr) > -1);
...
let getLocalDate = (date, isSeconds = false, isISO = false) => {
  ...
  if (isSeconds === false && isISO === false) {
    return date.toLocaleString().slice(0,-3);
  } else if (isSeconds === true && isISO === false) {
